...
add_command_under_category 'backup', 'general', 'Backup the CINC Server', 2 do
  ...
  OptionParser.new do |opts|
    opts.banner = 'Usage: cinc-server-ctl backup [options]'
...
add_command_under_category 'restore', 'general', 'Restore the CINC Server from backup', 2 do
  ...
  OptionParser.new do |opts|
    opts.banner = 'Usage: cinc-server-ctl restore $PATH_TO_BACKUP_TARBALL [options]'
  ...
  unless ARGV.length >= 2
    ...
    log('USAGE: cinc-server-ctl restore $PATH_TO_BACKUP_TARBALL [options]', :error)
    exit(1)
  end
