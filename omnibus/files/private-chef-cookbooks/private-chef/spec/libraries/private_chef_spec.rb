  ...
  context 'Key File Migration' do
    ...
    describe '#migrate_keys' do
      it 'should attempt to migrate known keys' do
        expect(PrivateChef).to receive(:add_key_from_file_if_present).with("#{Chef::Dist::SERVER}", 'superuser_key', superuser_key_path)
        expect(PrivateChef).to receive(:add_key_from_file_if_present).with("#{Chef::Dist::SERVER}", 'webui_key', webui_key_path)
