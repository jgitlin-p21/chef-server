...
add_command_under_category "grant-server-admin-permissions", "server-admins", "Grant a user the ability to create other users by adding the user to the server-admins group.", 2 do
  ...
  puts "User #{username} was added to server-admins. This user can now list, read, create, and delete users (even for orgs they are not members of) for this CINC Server."
end
...
add_command_under_category "remove-server-admin-permissions", "server-admins", "Remove all special permission granted to a user from being a server-admin.", 2 do
  ...
  puts "User #{username} was removed from server-admins. This user can no longer list, read, create, and delete users for this CINC Server except for where they have default permissions (such as within an org)."
...
def get_server_admins_authz_id(db)
  ...
  if server_admins_erchef_group.ntuples == 0
    msg = "The server-admins global group was not found. Please finish upgrading your CINC Server by following the documentation before using Server Admins related commands."
    STDERR.puts msg
    raise SystemExit.new(1, msg)
  end
