...
def secrets_apply_loop
  toml_cfg = TOML.load_file("/hab/svc/#{Chef::Dist::SERVER}-ctl/config/hab-secrets-config.toml")
  ...
  if changes_to_apply
    ...
    cmd = "hab config apply #{Chef::Dist::SERVER}-ctl.default #{version} {{pkg.svc_data_path}}/hab-secrets-modified.toml"
