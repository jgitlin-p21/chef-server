...
add_command_under_category 'backup', 'general', 'Backup the Chef Server', 2 do
  ...
  OptionParser.new do |opts|
    opts.banner = "Usage: #{Chef::Dist::SERVER_CTL} backup [options]"
...
add_command_under_category 'restore', 'general', 'Restore the Chef Server from backup', 2 do
  ...
  OptionParser.new do |opts|
    opts.banner = "Usage: #{Chef::Dist::SERVER_CTL} restore $PATH_TO_BACKUP_TARBALL [options]"
  ...
  unless ARGV.length >= 2
    ...
    log("USAGE: #{Chef::Dist::SERVER_CTL} restore $PATH_TO_BACKUP_TARBALL [options]", :error)
    exit(1)
  end
