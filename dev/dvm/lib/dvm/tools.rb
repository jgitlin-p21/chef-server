module DVM
  module Tools
    ...
    def clone(name, uri)
      run_command("git clone '#{uri}' '#{name}'", "Cloning #{name} to host. For future reference, you may also symlink it into #{Chef::Dist::SERVER}/external-deps from another location on the host.",
