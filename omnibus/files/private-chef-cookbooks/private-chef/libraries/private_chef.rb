...
module PrivateChef
  ...
  class << self
        ...
        if command_name
          warning << <<~WARN2
            ...
            chef-server.rb and setting the correct value with:
                ...
                #{node['wordmarks']['server']['ctl']} #{command_name}
          WARN2
        end
    ...
    def migrate_keys
      did_something = add_key_from_file_if_present("#{node['wordmarks']['server']['short']}", 'superuser_key', '/etc/opscode/pivotal.pem')
      did_something |= add_key_from_file_if_present("#{node['wordmarks']['server']['short']}", 'webui_key', '/etc/opscode/webui_priv.pem')
