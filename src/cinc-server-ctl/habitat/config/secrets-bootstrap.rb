...
def secrets_apply_loop
  toml_cfg = TOML.load_file('/hab/svc/cinc-server-ctl/config/hab-secrets-config.toml')
  ...
  if changes_to_apply
    ...
    cmd = "hab config apply cinc-server-ctl.default #{version} {{pkg.svc_data_path}}/hab-secrets-modified.toml"
