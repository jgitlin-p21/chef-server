...
if File.exist?("/etc/opscode/#{Chef::Dist::SERVER}-running.json")
  old_config = JSON.parse(IO.read("/etc/opscode/#{Chef::Dist::SERVER}-running.json"))
...
if File.exist?("/etc/opscode/#{Chef::Dist::SERVER}.json") &&
   !(File.exist?('/etc/opscode/private-chef.rb') || File.exist?("/etc/opscode/#{Chef::Dist::SERVER}.rb"))
  Chef::Log.fatal("Configuration via /etc/opscode/#{Chef::Dist::SERVER}.json is not supported. Please use /etc/opscode/#{Chef::Dist::SERVER}.rb")
  exit!(1)
else
  PrivateChef[:node] = node
  private_chef_path = '/etc/opscode/private-chef.rb'
  chef_server_path = "/etc/opscode/#{Chef::Dist::SERVER}.rb"
  ...
  if private_chef_rb_exists && private_chef_rb_not_symlink && chef_server_rb_exists &&
    ...
    Chef::Log.warn("/etc/opscode/private-chef.rb is deprecated and should be removed. Using /etc/opscode/#{Chef::Dist::SERVER}.rb")
  elsif private_chef_rb_exists && private_chef_rb_not_symlink
    Chef::Log.warn("Moving to /etc/opscode/#{Chef::Dist::SERVER}.rb for configuration - /etc/opscode/private-chef.rb is deprecated.")
  ...
  if File.exist?("/etc/opscode/#{Chef::Dist::SERVER}.json")
    Chef::Log.warn("Ignoring unsupported configuration file /etc/opscode/#{Chef::Dist::SERVER}.json.")
    Chef::Log.warn("Using /etc/opscode/#{Chef::Dist::SERVER}.rb instead.")
  end
