module DVM
  ...
  class RubyProject < Project
    ...
    def run(args)
      if @project['system']
        raise DVM::DVMArgumentError, "Run not supported for system ruby projects - just use it normally via #{Chef::Dist::SERVER_CTL} or otherwise, as it has been loaded into the server gemset."
      else
