...
node.default["#{Chef::Dist::SERVER}-deploy"]['automate_server_fqdn'] = server_fqdn_for('automate')
node.default["#{Chef::Dist::SERVER}-deploy"]['chef_server_fqdn'] = server_fqdn_for('#{Chef::Dist::SERVER}')
node.default["#{Chef::Dist::SERVER}-deploy"]['supermarket_fqdn'] = server_fqdn_for('supermarket')
node.default["#{Chef::Dist::SERVER}-deploy"]['enable_liveness_agent'] = environment == 'delivered'
...
cert_filename = "/etc/opscode/#{node["#{Chef::Dist::SERVER}-deploy"]['chef_cert_filename']}"
key_filename  = "/etc/opscode/#{node["#{Chef::Dist::SERVER}-deploy"]['chef_key_filename']}"
...
file cert_filename do
  mode '0600'
  content citadel[node["#{Chef::Dist::SERVER}-deploy"]['chef_cert_filename']]
end

file key_filename do
  mode '0600'
  content citadel[node["#{Chef::Dist::SERVER}-deploy"]['chef_key_filename']]
end

if node["#{Chef::Dist::SERVER}-deploy"]['enable_liveness_agent']
  remote_file automate_liveness_recipe_path do
    source liveness_agent_recipe_url
    mode '644'
    notifies :reconfigure, "chef_ingredient[#{Chef::Dist::SERVER}]"
  end
end

chef_ingredient "#{Chef::Dist::SERVER}" do
  channel omnibus_channel_for_environment("#{Chef::Dist::SERVER}")
  version version_for_environment("#{Chef::Dist::SERVER}")
...
template "/etc/opscode/#{Chef::Dist::SERVER}.rb" do
  ...
  variables(
    chef_server_deploy: node["#{Chef::Dist::SERVER}-deploy"],
  ...
  notifies :reconfigure, "chef_ingredient[#{Chef::Dist::SERVER}]", :immediately
end
...
# Chef Manage
################################################################################
node.default["#{Chef::Dist::SERVER}-deploy"]['manage-config']['saml.enabled'] = node["#{Chef::Dist::SERVER}-deploy"]['enable_saml']
node.default["#{Chef::Dist::SERVER}-deploy"]['manage-config']['saml.issuer_url'] = "'https://#{node["#{Chef::Dist::SERVER}-deploy"]['automate_server_fqdn']}/api/v0'"

chef_ingredient 'manage' do
  channel :stable
  accept_license true
  config node['#{Chef::Dist::SERVER}-deploy']['manage-config'].map { |k, v| "#{k} #{v}" }.join("\n")
  action :upgrade
  # We need to reconfigure after an install/upgrade
  notifies :reconfigure, 'chef_ingredient[manage]'
end

if node['#{Chef::Dist::SERVER}-deploy']['enable_saml']
...
omnibus_service '#{Chef::Dist::SERVER}' do
  action :nothing
end
...
omnibus_service '#{Chef::Dist::SERVER}/nginx' do
  action :nothing
end
...
EOF
  ...
  notifies :restart, 'omnibus_service[#{Chef::Dist::SERVER}/nginx]'
end
...
# Users
{
  ...
  execute "add-#{user_name}-user-key" do
    command lazy { "#{Chef::Dist::SERVER}-ctl add-user-key #{user_name} --public-key-path /etc/opscode/users/#{user_name}-2.pem.pub --key-name #{user_name}-2" }
    not_if "#{Chef::Dist::SERVER}-ctl list-user-keys #{user_name} | grep #{user_name}-2"
  end
end
