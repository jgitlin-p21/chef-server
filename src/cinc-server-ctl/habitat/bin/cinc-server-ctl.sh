...
pkg_prefix=$(cat /hab/svc/cinc-server-ctl/config/pkg_path)
...
export CHEF_SECRETS_DATA=$(cat /hab/svc/cinc-server-ctl/config/hab-secrets-config.json)
export CSC_KNIFE_CONFIG_FILE=/hab/svc/cinc-server-ctl/config/pivotal.rb
export CSC_KNIFE_BIN="${BUNDLE_BIN_DIR}/bundle exec ${pkg_prefix}/chef/bin/knife"

bundle exec binstubs/cinc-server-ctl "$@"
