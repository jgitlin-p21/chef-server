...
unless PrivateChef.credentials.exist?('cinc-server', 'superuser_key')
  pivotal_key = OpenSSL::PKey::RSA.generate(2048)
  PrivateChef.credentials.add('cinc-server', 'superuser_key',
...
if !PrivateChef.credentials.exist?('cinc-server', 'webui_key')
  webui_key = OpenSSL::PKey::RSA.generate(2048)
  PrivateChef.credentials.add('cinc-server', 'webui_key',
    value: webui_key.to_pem,
    frozen: true)
  # Store the public key in its own key for easy access
  PrivateChef.credentials.add('cinc-server', 'webui_pub_key',
    value: webui_key.public_key.to_s,
    frozen: true)
  PrivateChef.credentials.save
elsif !PrivateChef.credentials.exist?('cinc-server', 'webui_pub_key')
  webui_string = PrivateChef.credentials.get('cinc-server', 'webui_key')
...
webui_key = OpenSSL::PKey::RSA.new(PrivateChef.credentials.get('cinc-server', 'webui_key'))
...
if node['private_chef']['insecure_addon_compat']
  file '/etc/opscode/pivotal.pem'  do
    ...
    mode '0600'
    sensitive true
    content PrivateChef.credentials.get('cinc-server', 'superuser_key')
  end
