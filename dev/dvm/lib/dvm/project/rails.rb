...
EOM
    ...
    def enable_service
      run_command("cinc-server-ctl start #{service['name']}", "Starting #{omnibus_project} #{service['name']}")
    end

    def disable_service
      run_command("cinc-server-ctl stop #{service['name']}", "Stopping #{omnibus_project} #{service['name']}")
    end
