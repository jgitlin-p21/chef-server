...
EOF
    ...
    file_open("/etc/opscode/#{Chef::Dist::SERVER}.rb", "a"){ |file| file.write(content) }
  ...
  def fix_rabbit_wait_script
    ...
    script = "/opt/#{Chef::Dist::SERVER}/bin/wait-for-rabbit"
    sed = "sed 's/opscode/#{Chef::Dist::SERVER}/g' #{script} > #{script}.tmp && cp --no-preserve=mode,ownership #{script}.tmp #{script} && rm #{script}.tmp"
  ...
  def start_chef11
    ...
    check_status(run_command("/opt/#{Chef::Dist::SERVER}/bin/#{Chef::Dist::SERVER_CTL} start"), msg)
    wait_for_ready_server("Chef 11")
  end
  ...
  def pull_chef11_db_credentials
    ...
    if !File.exists?("/etc/#{Chef::Dist::SERVER}/#{Chef::Dist::SERVER}-running.json")
      log "Failed to find /etc/#{Chef::Dist::SERVER}/#{Chef::Dist::SERVER}-running.json"
      exit 1
    end

    running_config = JSON.parse(File.read("/etc/#{Chef::Dist::SERVER}/#{Chef::Dist::SERVER}-running.json"))
  ...
  def stop_chef11
    ...
    status = run_command("/opt/#{Chef::Dist::SERVER}/bin/#{Chef::Dist::SERVER_CTL} stop")
    check_status(status, msg)
  end
  ...
  def write_knife_ec_backup_config
    ...
    key.puts credentials.get("#{Chef::Dist::SERVER}", "superuser_key")
    key.flush
    key.close
  ...
  def upgrade_success_message(chef11_data_dir, chef12_data_dir)
    ...
    log "The Chef 12 server package (#{Chef::Dist::SERVER}-core) has been successfully setup."
    log "The Chef 11 server package (#{Chef::Dist::SERVER}) is still present on the system and can now be safely removed."
